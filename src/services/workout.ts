...
export const generateTreadmillWorkout = (speed: string,
    incline: string,
    ...
    for (let i = 0; i < trackIds.length; i++) {
        const incline: string = roundToHalf((inclineRange * trackEnergies[i]) + inclineMin).toString();
        const speed_kmph: string = ((speedRange * trackEnergies[i]) + speedMin).toFixed(1);
        const speed_mph: string = (((speedRange * trackEnergies[i]) + speedMin) / 1.609).toFixed(1);
        const speed = new Speed(speed_kmph, speed_mph);
        const track = new Track(trackIds[i], trackNames[i], trackEnergies[i].toString(), incline, speed)
        tracks.push(track);
    }
    console.log(tracks)
    return tracks;
