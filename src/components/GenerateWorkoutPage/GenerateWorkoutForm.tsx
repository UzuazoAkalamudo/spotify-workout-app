...
const GenerateWorkoutForm = ({ setWorkoutData }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      const tracks = generateTreadmillWorkout(
        formData.speed,
        formData.incline,
        trackNames,
        trackIds,
  ...
  return (
    <section className="flex flex-col justify-center items-center h-screen w-screen">
      <form
        aria-labelledby="treadmill-workout-form"
        id="treadmill-workout-form"
        onSubmit={handleSubmit}
        className="flex flex-col w-96 gap-8 px-20 py-10 rounded-xl"
      >
        <div className="flex relative flex-col gap-2">
          <input
            aria-label="Search for an album"
            className="w-full p-2 border-2 border-gray-300 rounded-lg text-gray-700 bg-gray-100 focus:border-blue-400 focus:bg-white focus:outline-none"
            id="search-input"
            name="searchInput"
          ...
          {searchItemsVisible && (
            <ul
              id="album-list"
              className="flex w-full absolute mt-11 flex-col gap-2 max-h-80 overflow-y-auto bg-gray-300 rounded-lg"
            >
        ...
        <div>
          <button
            aria-label="Generate your treadmill workout based on selected album and preferences"
