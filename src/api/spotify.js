  ...
  if (searchTextLength > 0) {
    try {
      const response = await fetch(
        `https://api.spotify.com/v1/search?q=album%3A${searchInput}&type=album`,
        {
          headers: {
...
export const getSpotifyAlbumTracks = async (album_id, token) => {
  try {
    const response = await fetch(
      `https://api.spotify.com/v1/albums/${album_id}/tracks?limit=50&offset=0`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
export const getSpotifyAlbumTracksData = async (track_ids, token) => {
  ...
  try {
    const response = await fetch(
      `https://api.spotify.com/v1/audio-features?ids=${track_ids_appended}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
    ...
    const data = await response.json();
    console.log(`Track analysis: ${data}`);
    return data;
  } catch (error) {
